 ...
 * Lucas Koehler - adapt to segments
 ******************************************************************************/
	...
	protected void onSelectButton(Label control) {
		...
		if (eContainer.getDomainModelReference() == null) {
			MessageDialog.openError(control.getShell(), "No Domain Model Reference found", //$NON-NLS-1$
				"A Domain Model Reference needs to be added to the Stack Layout first. " //$NON-NLS-1$
			);
			return;
		}

		final EClass rootEClass = Helper.getRootEClass(stackItem);
		IValueProperty valueProperty;
		try {
			valueProperty = Activator.getDefault().getEMFFormsDatabinding()
				.getValueProperty(eContainer.getDomainModelReference(), rootEClass);
		} catch (final DatabindingFailedException ex) {
			getReportService().report(new DatabindingFailedReport(ex));
			return;
		}
